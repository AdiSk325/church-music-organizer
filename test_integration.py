...
def test_imports():
    ...
    try:
        from src.ocr import SheetMusicOCR, MusicXMLConverter  # noqa: F401
        print("[OK] OCR module imported successfully")
    except ImportError as e:
        print(f"[FAIL] Failed to import OCR module: {e}")
        return False
...
def test_database():
    ...
    from src.database import get_db_session, MusicPiece, MusicFile, Tag, FileType  # noqa: F401
    ...
    from src.database.database import create_engine, Base, SessionLocal  # noqa: F401
    ...
    try:
        ...
        piece_id = piece.id
        session.close()
        print(f"[OK] Created music piece (ID: {piece_id})")
        ...
        session.commit()
        session.close()
        print("[OK] Added tag to music piece")

        # Test adding a file (path does not need to exist for the DB record)
        session = TestSession()
        ...
        session.commit()
        session.close()
        print("[OK] Added file to music piece")
        ...
        session.close()
        print(f"[OK] Query returned {len(pieces)} piece(s)")
        ...
        # Dispose engine to release SQLite file locks (important on Windows).
        engine.dispose()
        if os.path.exists(test_db_path):
            try:
                os.remove(test_db_path)
...
def main():
    ...
    for test_name, test_func in tests:
        ...
        try:
            ...
            print(f"\n[FAIL] Test '{test_name}' raised an exception: {e}")
    ...
    if passed == total:
        print("\n[ALL PASSED] The application is ready to use.")
        return 0
    else:
        print(f"\n[FAILURES] {total - passed} test(s) failed. Please check the errors above.")
        return 1
